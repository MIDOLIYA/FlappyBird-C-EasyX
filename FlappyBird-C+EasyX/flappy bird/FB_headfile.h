...
{
	IMAGE birdpt[20][2];//bird图片
	IMAGE bandpt[2];//障碍物图片
	IMAGE backgroundpt[2];//背景图片
	IMAGE landpt;//下方可以移动土地
	IMAGE big_num[10][2];//大号的数字
	IMAGE mid_num[10][2];//中号数字
	IMAGE sm_num[10][2];//小号数字
...
void picturedeal(Picture &picture);//游戏图片处理（加载与旋转） n=1 是加载图片
void drawpicture(Picture picture, Data data);//绘制游戏内容
